...
public class CustomerIdFilter implements QueryField {
    ...
    public void addToQuery(BooleanBuilder booleanBuilder, SearchCriteria searchCriteria) {
        List<CustomerModelApi> customerResponses = customerApiRestCallService.getCustomers((String) searchCriteria.getValue());
        List<String> customers = new ArrayList<>();
        customerResponses.stream().forEach(customerModelApi -> customers.add(customerModelApi.getCustomerId()));
        searchCriteria.setValue(customers);
        booleanBuilder.and(predicateBuilder.getBooleanBuilder(QInvoiceDataEntity.invoiceDataEntity.customerId, searchCriteria));
    }
}
