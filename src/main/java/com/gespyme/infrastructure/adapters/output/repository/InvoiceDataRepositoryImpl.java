...
public class InvoiceDataRepositoryImpl implements InvoiceDataRepository {
    ...
    public InvoiceDataRepositoryImpl(List<QueryField> queryFields, InvoiceDataRepositorySpringJpa invoiceDataRepositorySpringJpa, JPAQueryFactory queryFactory, InvoiceDataMapper mapper, CustomerApiRestCallService customerApiRestCallService) {
    ...
    public List<InvoiceData> findByCriteria(List<SearchCriteria> searchCriteria) {
        ...
        JPAQuery<InvoiceDataEntity> query = queryFactory.select(invoiceData).from(invoiceData).join(invoiceData.invoiceOrder, invoiceOrder);
        BooleanBuilder booleanBuilder = getPredicates(searchCriteria);
        List<Tuple> result = Objects.nonNull(booleanBuilder.getValue()) ? executeQueryWithPredicate(invoiceData, invoiceOrder, booleanBuilder, query) : executeQueryWithoutPredicate(invoiceData, invoiceOrder, query);
        return result.stream().map(tuple -> mapTuple(tuple, invoiceData, invoiceOrder)).collect(Collectors.toList());
    ...
    public InvoiceData save(InvoiceData invoiceData) {
        InvoiceDataEntity invoiceDataEntity = invoiceDataRepositorySpringJpa.save(mapper.mapToEntity(invoiceData));
